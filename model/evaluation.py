...
class Evaluator:
    ...
    def log_acc(self,idx,datalen,average=False):
        if average:
            ...
            logging.info(' * Average Acc: %3.3f' % avg_acc)
            logging.info(' * Normalized Average Acc: %3.3f' % normalized_avg_acc)

            return avg_acc, normalized_avg_acc


        logging.info('[%5d/%5d]: \t [Acc: %3.3f]\t[Average Acc: %3.3f] \t[Normalized Average Acc: %3.3f]' %
                     (idx + 1,
                      datalen,
